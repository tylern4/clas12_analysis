...
Histogram::Histogram() {
  ...
  W_vs_q2 =
      new TH2D("W_vs_q2", "W_vs_q2", bins, zero, w_max, bins, zero, q2_max);
...
void Histogram::makeHists_sector() {
  for (size_t i = 0; i < num_sectors; i++) {
    ...
    W_vs_q2_sec[i] = new TH2D(hname.c_str(), htitle.c_str(), bins, zero, w_max,
                              bins, zero, q2_max);
    hname.clear();
    htitle.clear();
    ...
    htitle.append(std::to_string(i + 1));
    MM_neutron_sec[i] =
        new TH1D(hname.c_str(), htitle.c_str(), bins, zero, 4.0);
    hname.clear();
    htitle.clear();
  }
}
  ...
  for (size_t p = 0; p < particle_num; p++) {
    for (size_t c = 0; c < charge_num; c++) {
      for (size_t i = 0; i < with_id_num; i++) {
        ...
        if (delta_t_hist[p][c][i]->GetEntries() > 1)
