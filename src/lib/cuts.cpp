...
bool uconn_Cuts::DC_fiducial_cut_XY(int num, int partpid) {
  ...
  int dc_sector = _data->dc_sec(num);
  if (dc_sector < 0) return false;

  // float maxparams[6][6][3][2];
  // float minparams[6][6][3][2];
  // Inbending
  float maxparams[6][6][3][2] = {{{{-14.563, 0.60032}, {-19.6768, 0.58729}, {-22.2531, 0.544896}},
  ...
  float minparams[6][6][3][2] = {{{{12.2692, -0.583057}, {17.6233, -0.605722}, {19.7018, -0.518429}},
  ...
  for (short reg = 1; reg < 3; reg++) {
    ...
    float calc_min = minparams[pid][dc_sector - 1][reg - 1][0] + minparams[pid][dc_sector - 1][reg - 1][1] * X;
    float calc_max = maxparams[pid][dc_sector - 1][reg - 1][0] + maxparams[pid][dc_sector - 1][reg - 1][1] * X;
    _cut &= (Y > calc_min) && (Y < calc_max);
  }
